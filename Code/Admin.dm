...
#define DENY_VAR_EDIT 	list("ckey", "key","parent_type","type") //  Shows these variables in the edit list BUT it is uneditable.
...
#define check_Initialized var/turf/m=locate("t")
...
//definitions
#define SPAWN_RATE		600		//(seconds * 10)
...
mob/verb
	Edit()
		...
		if(usr.Admin==0)
			return
		var/tmp/Edit=list()
		for(var/A as mob in world)
			Edit+=A
		var/E=input("Who Would You Like To Edit","Edit") in Edit + list("Cancel")
		if(E=="Cancel")
			return
		else
			src.client<<link("?command=edit;target=\ref[E];type=view;")
			return
	Reboot()
		set hidden = 1
		if(usr.Admin==0)
			return
		switch(input("Your Sure You Wanna Reboot","Reboot") in list("Yes","No"))
			if("Yes")
				world.Reboot()
			if("No")
				return
	...
	Mute(mob/M in world)
		...
		if(usr.Admin==0)
			...
			if(M.Mute)
				M.Mute=0
				_message(M, "<b><font color=#7F00FF>[M.Name]</font>: You're UnMuted</b>","white")
			else
				M.Mute=1
